...
const AdminProduct = () => {
  ...
  const handlesave = async (e) => {
    ...
    try {
      const res = await axios.put(`https://shopping-ekart-backend.onrender.comapi/v1/product/update/${editProduct._id}`, formData, {
  ...
  const deleteProductHandler = async (productId) => {
    try {
      ...
      const res = await axios.delete(`https://shopping-ekart-backend.onrender.comapi/v1/product/delete/${productId}`, {
        headers: { Authorization: `Bearer ${accessToken}` }
      })
      if (res.data.success) {
