...
const Address = () => {
  ...
  const handlePayment = async () => {
    try {
      const { data } = await axios.post('/api/v1/orders/create-order', {
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            const verifyRes = await axios.post('/api/v1/orders/verify', response,
              { headers: { Authorization: `Bearer ${accessToken}` } })
        ...
        modal: {
          ondismiss: async function () {
            await axios.post('/api/v1/orders/verify',
